...
void u8g2_prepare(void)
{
    ...
    tft.drawString("Preparing everything", 0, 0, 2);
}
...
{
    debugPrintf("Display Message: %s %s %s\n", text1.c_str(), text2.c_str(), text3.c_str());
    tft.fillScreen(TFT_BLACK);
    tft.setTextColor(TFT_WHITE);
    tft.drawString("Fehler", 0, 10, 3);
    //  u8g2.clearBuffer();
    //  u8g2.setCursor(0, 0);
    //  u8g2.print(text1);
    //  u8g2.setCursor(0, 10);
...
{
    if ((machineState == kBrew) && SHOTTIMER == 1)
    { // Shotimer has to be 1 and brew is running, then show time
      // u8g2.clearBuffer();
